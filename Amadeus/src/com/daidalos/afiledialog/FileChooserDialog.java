...
public class FileChooserDialog extends Dialog implements FileChooser {
	...
	/**
	 * Creates a file chooser dialog which lists all the file of a particular
	 * folder.
	 * 
	 * @param context
	 *            The current context.
	 * @param folderPath
	 *            The folder which files are going to be listed.
	 */
		...
		core.addListener(new FileChooserCore.OnFileSelectedListener() {
			public void onFileSelected(File folder, String name) {
				...
				for (int i = 0; i < FileChooserDialog.this.listeners.size(); i++) {
					FileChooserDialog.this.listeners.get(i).onFileSelected(
							FileChooserDialog.this, folder, name);
				}
			}
	...
	/**
	 * Allows to define if a confirmation dialog must be show when selecting o
	 * creating a file.
	 * 
	 * @param onSelect
	 *            'true' for show a confirmation dialog when selecting a file,
	 *            'false' if not.
	 * @param onCreate
	 *            'true' for show a confirmation dialog when creating a file,
	 *            'false' if not.
	 */
