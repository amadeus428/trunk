...
public class FileItem extends LinearLayout {
	 ...
	 * 
	 * @param selectable
	 *            'true' if the item can be selected, 'false' if not.
	 */
	...
	/**
	 * Updates the icon according to if the file is a folder and if it can be
	 * selected.
	 */
	private void updateIcon() {
		...
		if (this.selectable) {
			icon = (this.file != null && file.isDirectory()) ? R.drawable.folder
					: R.drawable.document;
		}
		...
		if (icon != R.drawable.document_gray) {
			this.label.setTextColor(getResources().getColor(
					R.color.daidalos_active_file));
		} else {
			this.label.setTextColor(getResources().getColor(
					R.color.daidalos_inactive_file));
		}
	}
	...
	/**
	 * Interface definition for a callback to be invoked when a FileItem is
	 * clicked.
	 */
