...
import com.daidalos.afiledialog.FileItem;
...
class FileChooserCore {
    ...
    /**
     * A boolean that indicates if a confirmation dialog must be displaying when
     * selecting a file.
     */
	...
	public void onClick(View v) {
	    ...
	    String title = context
		    .getString(FileChooserCore.this.folderMode ? R.string.daidalos_create_folder
			    : R.string.daidalos_create_file);
	    if (FileChooserCore.this.labels != null
		    && FileChooserCore.this.labels.createFileDialogTitle != null)
		title = FileChooserCore.this.labels.createFileDialogTitle;
	    String message = context
		    .getString(FileChooserCore.this.folderMode ? R.string.daidalos_enter_folder_name
			    : R.string.daidalos_enter_file_name);
	    if (FileChooserCore.this.labels != null
		    && FileChooserCore.this.labels.createFileDialogMessage != null)
		message = FileChooserCore.this.labels.createFileDialogMessage;
	    String posButton = (FileChooserCore.this.labels != null && FileChooserCore.this.labels.createFileDialogAcceptButton != null) ? FileChooserCore.this.labels.createFileDialogAcceptButton
		    : context.getString(R.string.daidalos_accept);
	    String negButton = (FileChooserCore.this.labels != null && FileChooserCore.this.labels.createFileDialogCancelButton != null) ? FileChooserCore.this.labels.createFileDialogCancelButton
	    ...
	    alert.setPositiveButton(posButton,
			...
			public void onClick(DialogInterface dialog,
				...
				FileChooserCore.this.notifyListeners(
					FileChooserCore.this.currentFolder,
					fileName);
			    }
			}
		    });
	    alert.setNegativeButton(negButton,
			...
			public void onClick(DialogInterface dialog,
				int whichButton) {
			    // Do nothing, automatically the dialog is going to
			    // be
			    // closed.
			}
		    });
	...
	public void onClick(View v) {
	    // Notify the listeners.
	    FileChooserCore.this.notifyListeners(
		    FileChooserCore.this.currentFolder, null);
	}
    };
	...
	if ((creation && this.showConfirmationOnCreate || !creation
		&& this.showConfirmationOnSelect)) {
	    // Create an alert dialog.
	    ...
	    String message = null;
	    if (FileChooserCore.this.labels != null
		    && ((creation && FileChooserCore.this.labels.messageConfirmCreation != null) || (!creation && FileChooserCore.this.labels.messageConfirmSelection != null))) {
		message = creation ? FileChooserCore.this.labels.messageConfirmCreation
			: FileChooserCore.this.labels.messageConfirmSelection;
	    } else {
		if (FileChooserCore.this.folderMode) {
		    message = context
			    .getString(creation ? R.string.daidalos_confirm_create_folder
				    : R.string.daidalos_confirm_select_folder);
		} else {
		    message = context
			    .getString(creation ? R.string.daidalos_confirm_create_file
				    : R.string.daidalos_confirm_select_file);
		}
	    }
	    if (message != null)
		message = message.replace("$file_name", name != null ? name
			: file.getName());
	    String posButton = (FileChooserCore.this.labels != null && FileChooserCore.this.labels.labelConfirmYesButton != null) ? FileChooserCore.this.labels.labelConfirmYesButton
		    : context.getString(R.string.daidalos_yes);
	    String negButton = (FileChooserCore.this.labels != null && FileChooserCore.this.labels.labelConfirmNoButton != null) ? FileChooserCore.this.labels.labelConfirmNoButton
	    ...
	    alert.setPositiveButton(posButton,
			...
			public void onClick(DialogInterface dialog,
				int whichButton) {
			    // Notify to listeners.
			    for (int i = 0; i < FileChooserCore.this.listeners
				    .size(); i++) {
				if (creation) {
				    FileChooserCore.this.listeners.get(i)
    ...
    /**
     * Allows to define if a confirmation dialog must be show when selecting a
     * file.
     * 
     * @param show
     *            'true' for show the confirmation dialog, 'false' for not show
     *            the dialog.
     */
    ...
    /**
     * Allows to define if a confirmation dialog must be show when creating a
     * file.
     * 
     * @param show
     *            'true' for show the confirmation dialog, 'false' for not show
    ...
    /**
     * Allows to define if, in the title, must be show only the current folder's
     * name or the full file's path..
     * 
     * @param show
	...
	// Verify if the buttons for add a file or select a folder has been
	// modified.
	if (labels != null) {
	    LinearLayout root = this.chooser.getRootLayout();
     ...
     * 
     * @param canCreate
     *            'true' if the user can create files or 'false' if it can only
     *            select them.
     */
    ...
    /**
     * Changes the height of the layout for the buttons, according if the
     * buttons are visible or not.
     */
    private void updateButtonsLayout() {
	// Get the buttons layout.
	LinearLayout root = this.chooser.getRootLayout();
	LinearLayout buttonsLayout = (LinearLayout) root
	...
	addButton.setVisibility(this.canCreateFiles ? View.VISIBLE
		: View.INVISIBLE);
	addButton.getLayoutParams().width = this.canCreateFiles ? ViewGroup.LayoutParams.MATCH_PARENT
		: 0;
	...
	okButton.getLayoutParams().width = this.folderMode ? ViewGroup.LayoutParams.MATCH_PARENT
		: 0;
	...
	if (this.canCreateFiles || this.folderMode) {
	    ...
	    // If only the 'Ok' button is visible, put him first. Otherwise, put
	    // 'Add' first.
	...
	LinearLayout layout = (LinearLayout) root
		.findViewById(R.id.linearLayoutFiles);
	...
	if (this.currentFolder.exists() && layout != null) {
		...
		if (parent.exists()) {
		    fileItems.add(new FileItem(this.chooser.getContext(),
			    parent, ".."));
		}
	    }
		...
		if (fileList != null) {
			...
			if (!fileList[i].isDirectory()) {
			    selectable = !this.folderMode
				    && (this.filter == null || fileList[i]
					    .getName().matches(this.filter));
			}
			...
			if (selectable || !this.showOnlySelectable) {
			    ...
			    FileItem fileItem = new FileItem(
				    this.chooser.getContext(), fileList[i]);
		...
		String currentFolderName = this.showFullPathInTitle ? this.currentFolder
			.getPath() : this.currentFolder.getName();
