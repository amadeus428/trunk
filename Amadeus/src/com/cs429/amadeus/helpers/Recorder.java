...
public abstract class Recorder {
	...
	public Recorder(StaffLayout staffLayout, float bpm) {
		...
		bps = bpm / 60.f;
		if (bps == 0)
			throw new IllegalArgumentException("bps is zero");
	 ...
	 * @param isPlayAlong
	 *            - is this function being called in a PlayAlong setting
	 */
		...
		if (currTime - lastRecordedNoteTime > period) {
			...
			if (note.octave >= 2 && note.octave <= 8) {
				if (lastNote != null) {
					...
					if (isPlayAlong) {
						((PlayAlongStaffLayout) staffLayout)
								.addRecordedNote(lastNote);
					} else {
