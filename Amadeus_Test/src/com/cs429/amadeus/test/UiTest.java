...
public class UiTest extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	public void testDrawer() throws InterruptedException {
		...
		mStaffLayout = (StaffLayout) mActivity.findViewById(R.id.fragment_recording_staff_layout);
		assertNotNull(mStaffLayout);
		
		selectDrawer(R.string.title_play_along);
		mStaffLayout = (StaffLayout) mActivity.findViewById(R.id.fragment_play_along_staff_layout);
		Assert.assertNotNull(mStaffLayout);
		
		selectDrawer(R.string.title_guitar_chords);
		mImage = (ImageView) mActivity.findViewById(R.id.fragment_guitar_chords_chord_image);
		...
		Button button = (Button)mActivity.findViewById(R.id.fragment_home_play_along_button);
	...
	private void selectDrawer(final int id) {
		mActivity.runOnUiThread(new Runnable() {
			public void run() {
				 mActivity.replaceContentViewOnItemSelected(mActivity.getIndexOfItemInDrawer(id));
			}
		});
	...
	public void testOnDrawCanvas() {
		...
		mActivity.runOnUiThread(new Runnable() {
			public void run() {
				((Button) mActivity.findViewById(R.id.fragment_home_create_sheet_music_button)).performClick();
	...
	public void testPlayAlong() {
		...
		mActivity.runOnUiThread(new Runnable() {
			public void run() {
				((Button)mActivity.findViewById(R.id.fragment_home_play_along_button)).performClick();
			}
		});
		mInstrumentation.waitForIdleSync();
		mStaffLayout = (StaffLayout) mActivity.findViewById(R.id.fragment_play_along_staff_layout);
	...
	public void testGuitar() {    
		...
		mActivity.runOnUiThread(new Runnable() {
			public void run() {
				((Button)mActivity.findViewById(R.id.fragment_home_guitar_chords_button)).performClick();
			}
		});
		mInstrumentation.waitForIdleSync();
		mImage = (ImageView) mActivity.findViewById(R.id.fragment_guitar_chords_chord_image);
		...
		mSpinner = (Spinner) mActivity.findViewById(R.id.fragment_guitar_chords_chord_spinner);
		mActivity.runOnUiThread(new Runnable() {
			public void run() {
		...
		for (int i=0;i<15;i++) {
			...
			assertEquals(id,R.drawable.a + pos);
